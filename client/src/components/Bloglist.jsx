...
const Bloglist = () => {
  ...
  return (
    <div>
      <div className='flex flex-wrap justify-center gap-x-4 gap-y-2 sm:gap-8 my-10 relative px-4 sm:px-8'>
        {blogCategories.map((item)=>(
          <div key={item} className='relative'>
            <button onClick={()=> setMenu(item)} className={`cursor-pointer text-gray-500 text-sm py-1.5 px-3 rounded-full ${menu === item && 'text-white px-4 pt-0.5'}`}>
              {item}
              {menu === item && (
